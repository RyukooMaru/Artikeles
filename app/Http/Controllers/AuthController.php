<?php

namespace App\Http\Controllers;

use Illuminate\Http\Request;
use Illuminate\Support\Facades\Hash;
use App\Models\User;
use App\Models\Users;
use Database\Seeders\userseeder;

class AuthController extends Controller
{
    public function showAuthPage()
    {
        if (session()->has('usersid')) {
            return view('authes.authes')->with('form', 'captcha');
        }
    
        return view('authes.authes');
    }
    public function showMain(Request $request)
    {
        return view('appes.artikeles');
    }
    // LOGIN
    public function login(Request $request)
    {
        $request->validate([
            'username' => 'required|string',
            'password' => 'required|string',
        ]);

        $username = $request->input('username');
        $password = $request->input('password');

        $user = User::whereRaw('BINARY `username` = ?', [$username])->first();
        if ($user && Hash::check($password, $user->password)) {
            session([
                'isLoggedIn' => true,
                'userid' => $user->userid, 
                'username' => $user->username,
                'nameuse' => $user->nameuse,
                'improfil' => $user->improfil,
            ]);
            
            return redirect('/artikeles')->with('success', 'Berhasil login!');
        }
        return redirect()->route('authes')
            ->with('error', 'Username atau Password salah!')
            ->with('form', 'login');
    }
    public function logout()
    {
        session()->flush();
        return redirect()->route('authes')->with('success', 'Berhasil logout!')->with('form', 'login');
    }

    public function verifcapt(Request $request)
    {
        $usersid = session('usersid'); // Ambil session usersid

        // Cari data Users berdasarkan ID
        $user = Users::find($usersid);

        // Cek apakah user ditemukan
        if (!$user) {
            return redirect()->route('authes')
                ->with('error', 'User tidak ditemukan!')
                ->with('form', 'captcha');
        }

        // Kalau captcha sudah diverifikasi, langsung ke halaman artikeles
        if ($user->captcha_verified) {
            return redirect()->route('appes.artikeles');
        }

        // Kalau belum, ubah status captcha_verified menjadi true
        $user->captcha_verified = true;
        $user->save();

        // Redirect ke halaman login setelah verifikasi captcha
        return redirect()->route('authes')
            ->with('success', 'Captcha berhasil diverifikasi! Silakan login.')
            ->with('form', 'login');
    }

    public function register(Request $request)
    {
        $username = $request->input('username');
        $nameuse = $request->input('nameuse');
        $password = $request->input('password');
        $password_confirmation = $request->input('password_confirmation');
        $defaultImage = 'data:image/png;base64,';
        

        $existingUser = User::where('username', $username)->first();

        if ($existingUser) {
            return redirect()->route('authes')
                ->with('registerError', 'Username sudah digunakan!')
                ->with('form', 'register')
                ->withInput();
        }

        if (mb_strlen($username) > 16) {
            return redirect()->route('authes')
                ->with('registerError', 'Username tidak boleh lebih dari 16 karakter!')
                ->with('form', 'register')
                ->withInput();
        }

        if (mb_strlen($nameuse) > 16) {
            return redirect()->route('authes')
                ->with('registerError', 'Nama Artikeles tidak boleh lebih dari 16 karakter!')
                ->with('form', 'register')
                ->withInput();
        }

        if ($password !== $password_confirmation) {
            return redirect()->route('authes')
                ->with('registerError', 'Password dan Konfirmasi Password tidak cocok!')
                ->with('form', 'register')
                ->withInput();
        }
        if (!session()->has('usersid')) {
            if (!session('Loggedin')) {
                return redirect()->route('authes')
                    ->with('captchaes', 'Selesaikan captcha terlebih dahulu!.')
                    ->with('form', 'captcha')
                    ->withInput();
            }   
        } else if(session('Loggedin')){
            return redirect()->route('authes')
                ->with('registerError', 'Gagal membuat akun!.')
                ->with('form', 'register')
                ->withInput();
        } else {
            return redirect()->route('authes')
                ->with('registerError', 'Gagal membuat akun!.')
                ->with('form', 'register')
                ->withInput();
        }
    }
    public function requestes(Request $request)
    {
        $userscreated = Users::create([
            'captcha_verified' => false,
        ]);

        $usersid = $userscreated->userid;

        session([
            'Loggedin' => true,
            'usersid' => $usersid,
            'captcha_verified' =>$userscreated->captchaverified
        ]);
        return redirect()->route('authes')
        ->with('captchaes', 'Selesaikan captcha terlebih dahulu!.')
        ->with('form', 'captcha')
        ->withInput();
    }
}

